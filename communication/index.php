	...
	function createRelations($node, &$solvers, &$relations, $prefix = ""){
		...
		if ($node->_type == "solver"){
			foreach(get_object_vars($node) as $k=>$v){
				if (!is_array($v))
					if (strpos($k, "_") === 0 && !in_array($k, array("_parent", "_name", "_comment", "_nested", "_id", "_type"))){
						...
						foreach($list as $k2 => $v2){
							if (isset($solvers[$v2])){
								...
								$type_arr[$k] = $k;
							} else if (isset($solvers[$prefix . $node->name . ":". $v2])){
								$relations[$prefix . $node->name . ":". $v2][$prefix . $node->name][$k] = 1;
								$type_arr[$k] = $k;								
							}							
						}
					}
...
?>	
		  ...
		  <?php } ?>		  
		  path_d += " " + points[1].x + "," + points[1].y;
		  el.append("path").attr("d", path_d).attr("fill", "none").attr("marker-end", "url(#inheritance-arrow)").attr("stroke-width" , "0.5").attr("class" , "<?php echo $k3?>");
